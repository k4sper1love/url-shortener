...
// SaveURL provides a mock function with given fields: targetURL, alias
func (_m *URLSaver) SaveURL(targetURL string, alias string) (int64, error) {
	ret := _m.Called(targetURL, alias)
	...
	if rf, ok := ret.Get(0).(func(string, string) (int64, error)); ok {
		return rf(targetURL, alias)
	}
	if rf, ok := ret.Get(0).(func(string, string) int64); ok {
		r0 = rf(targetURL, alias)
	} else {
		r0 = ret.Get(0).(int64)
	}

	if rf, ok := ret.Get(1).(func(string, string) error); ok {
		r1 = rf(targetURL, alias)
	} else {
		r1 = ret.Error(1)
	}
